...
function updateCommissionRegion(region) {
    ...
    loadJSON(`/locales/${lang}/commission.json?v=1.0.0`).then(data => {
...
function formatCommissionText(lang, achievement, commission, requirement, notes, video) {
    ...
    return [
        `<h2>${achievement}</h2>`,
        commission && `<h4 class="commission-section"><img src='/images/emotes/commission.webp' class='emote' alt='Commission'> ${texts.commission}</h4><p class="commission-text"> ${commission}</p>`,
        requirement && `<h4 class="commission-section"><img src='/images/emotes/condition.webp' class='emote' alt='Condition'> ${texts.requirement}</h4><p class="commission-text"> ${requirement}</p>`,
        notes && `<h4 class="commission-section"><img src='/images/emotes/notes.webp' class='emote' alt='Notes'> ${texts.notes}</h4> ${notes}`,
        video && `<p class="commission-section"><img src='/images/emotes/youtube.webp' class='emote' alt='Vidéo'> <strong>${texts.video}:</strong> ${video}</p>`
    ].filter(Boolean).join('');
}